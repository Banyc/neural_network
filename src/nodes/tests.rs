use crate::{
    ...
    nodes::input::InputNodeBatchParams, ref_ctr::RefCtr,
};
...
fn linear_evaluate() {
    ...
    let mut bias_node = bias_node(RefCtr::new(MutCell::new(weight_node)), initial_bias);
...
fn linear_with_relu_evaluate() {
    ...
    let initial_bias = RefCtr::new(MutCell::new(vec![initial_bias]));
    let weight_node = weight_node(input_nodes, initial_weights, None).unwrap();
    let bias_node = bias_node(RefCtr::new(MutCell::new(weight_node)), initial_bias);
    let bias_node = RefCtr::new(MutCell::new(bias_node));
    let mut relu_node = relu_node(RefCtr::clone(&bias_node));
    let mut cx = NodeContext::new();
