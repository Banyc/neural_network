...
pub trait NodeComputation {
    fn compute_output(&self, parameters: &Vec<f64>, operand_outputs: &Vec<f64>) -> f64;
...
impl GeneralNode {
    ...
    pub fn do_gradient_descent_step(&mut self, step_size: f64) {
    ...
    pub fn distribute_global_gradient_entries_to_operands(&mut self) {
    ...
    pub fn evaluate(&mut self, inputs: &Vec<f64>) -> f64 {
        self.cache.output.get_or_insert_with(|| {
            ...
            let ret = self
                .computation
                .compute_output(&self.parameters, &operand_outputs);
