...
fn linear_evaluate() {
    ...
    let mut bias_node = bias_node(Arc::new(RefCell::new(weight_node)), Some(initial_bias));
    let batch_index = 0;
...
fn linear_gradient_of_this_at_operand() {
    ...
    let ret = bias_node
        .gradient_of_this_at_operand(batch_index, &bias_node.parameters().borrow(), vec![])
        .unwrap();
    assert_eq!(&ret, &[1.0]);
}
...
fn linear_gradient_of_this_at_parameter() {
    ...
    bias_node.evaluate_once(&inputs, batch_index);
    let ret = bias_node
        .gradient_of_this_at_parameter(batch_index, &bias_node.parameters().borrow(), vec![])
        .unwrap();
    assert_eq!(&ret, &[1.0]);
}
...
fn linear_with_relu_evaluate() {
    ...
    let initial_bias = Arc::new(RefCell::new(vec![initial_bias]));
    let weight_node = weight_node(input_nodes, Some(initial_weights), None).unwrap();
    let bias_node = bias_node(Arc::new(RefCell::new(weight_node)), Some(initial_bias));
